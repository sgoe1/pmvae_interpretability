...
def plot_pretext_saliencies_lineplot(
    data: list, saliency: np.ndarray, pretext_names: list
) -> plt.Figure:
    n_pretext = len(pretext_names)
    n_plots = len(data)
    cblind_palette = sns.color_palette("colorblind")
    ...
    for example_id in range(n_plots):
        for pretext_id, pretext in enumerate(pretext_names):
            ...
            norm_saliency = (sub_saliency - np.min(sub_saliency)) / (
                np.max(sub_saliency) - np.min(sub_saliency)
            )

            # Create a scatter plot with color-coded saliency
            scatter = ax.scatter(
                range(len(data[example_id])),
                data[example_id],
                c=norm_saliency,
                cmap="viridis",
                alpha=0.6,
            ...
            # Optionally, add a line plot as well
            ax.plot(data[example_id], color="gray", alpha=0.5)
...
def plot_pretext_top_example_lineplot(
    train_data: list,
    test_data: list,
    example_importance: np.ndarray,
    pretext_names: list,
    ...
    n_pretext = len(pretext_names)
    n_plots = len(test_data)
    fig, axs = plt.subplots(
        ncols=n_pretext + 1, nrows=n_plots, figsize=(3 * (n_pretext + 1), 3 * n_plots)
    )

    for example_id in range(n_plots):
        # Plotting the test data as a line plot
        ax = axs[example_id, 0] if n_plots > 1 else axs[0]
        ax.plot(test_data[example_id])
        ax.axis("off")
        ax.set_title("Test Data")

        for pretext_id, pretext in enumerate(pretext_names):
            # Finding the index of the most important training example
            top_id = np.argmax(example_importance[pretext_id, example_id, :])

            # Plotting the corresponding training data as a line plot
            ax = axs[example_id, pretext_id + 1] if n_plots > 1 else axs[pretext_id + 1]
            ax.plot(train_data[top_id])
            ax.axis("off")
            ax.set_title(f"Top Data {pretext}")
