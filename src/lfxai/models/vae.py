        ...
        **kwargs,
        ...
        for i in list(range(len(hidden_layers))):
            if i == 0:
                ...
                module_name = "encoder_dense_%d" % (i + 1)
                self.add_module(
                    module_name, nn.Linear(hidden_layers[i - 1], hidden_layers[i] * 2)
                )
            else:
                module_name = "encoder_dense_%d" % (i + 1)
                self.add_module(
                    module_name, nn.Linear(hidden_layers[i - 1], hidden_layers[i])
                )
                if batch_norm:
                    module_name = "encoder_norm_%d" % (i + 1)
                    self.add_module(module_name, nn.BatchNorm1d(hidden_layers[i])),
        ...
        **kwargs,
        ...
        for i in list(range(len(reversedLayersList))):
            if i == (len(hidden_layers) - 1):
                ...
                self.add_module(
                    module_name, nn.Linear(reversedLayersList[i], n_features)
                )
            else:
                module_name = "decoder_dense_%d" % (i + 1)
                self.add_module(
                    module_name,
                    nn.Linear(reversedLayersList[i], reversedLayersList[i + 1]),
                )
                if batch_norm:
                    module_name = "decoder_norm_%d" % (i + 1)
                    self.add_module(
                        module_name, nn.BatchNorm1d(reversedLayersList[i + 1])
                    ),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if decoder == "neural":
            ...
            self.decoder_net = linearDecoder(
                n_features,
                hidden_layers,
                activation=activation,
                batch_norm=batch_norm,
...
class VAEModel(object):
    ...
    def train(
        ...
        for i_epoch in range(max_epochs):
            ...
            logstr = (
                f"Epoch {i_epoch:03d}: "
                + f"training loss {trainloss:08.4f},"
                + f"validation loss {valloss:08.4f}"
            )
    ...
    def _train_epoch(self, train_dataloader, use_c=False):
        ...
        for batch_idx, data in enumerate(train_dataloader):
            ...
            loss = self.loss_function(
                outputs.global_recon, x, outputs.mu, outputs.logvar
            )
    ...
    def _val_epoch(self, val_dataloader, use_c=False):
        ...
        for batch_idx, data in enumerate(val_dataloader):
            ...
            outputs = self.model(x)
            loss = self.loss_function(
                outputs.global_recon, x, outputs.mu, outputs.logvar, val=True
            )
    ...
    def get_recon_error(self, val_dataset, batch_size=256):
        val_dataloader = torch.utils.data.DataLoader(
            val_dataset, batch_size=batch_size, shuffle=False, num_workers=2
        )
...
class pmVAE(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoding_masks = build_mask_list(
        ...
        if cdim is not None:
            self.encoding_masks[0] = np.vstack(
                (
                    self.encoding_masks[0],
                    np.ones((cdim, self.encoding_masks[0].shape[1])),
                )
            )
            self.decoding_masks[0] = np.vstack(
                (
                    self.decoding_masks[0],
                    np.ones((cdim, self.decoding_masks[0].shape[1])),
                )
            )

        self.encoder_net = pmEncoder(
            membership_mask,
            hidden_layers,
            latent_dim,
            activation="elu",
        ...
        self.merge_layer = CustomizedLinear(
            self.decoding_masks[-1], bias=bias_last_layer
        )
    ...
    def get_masks_for_local_losses(self):
        if self.add_auxiliary_module:
            return zip(self.membership_mask[:-1], self.module_isolation_mask[:-1])

        return zip(self.membership_mask, self.module_isolation_mask)

    def forward(self, x, c=None, **kwargs):
        if self.training and self.input_pert:
            x = self.input_pert(x)
...
class pmVAEModel(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model = pmVAE(
            ...
            use_gpu=use_gpu,
            input_pert=input_pert,
            **kwargs,
        )

        self.encoder = self.model.encoder_net
        self.decoder = self.model.decoder_net
        ...
        if val:
            ...
            if pathway_dropout:
                for feat_mask, module_mask in self.model.get_masks_for_local_losses():
                    if self.use_gpu:
                        feat_mask, module_mask = (
                            torch.tensor(feat_mask).float().cuda(),
                            torch.tensor(module_mask).float().cuda(),
                        )
                    else:
                        feat_mask, module_mask = (
                            torch.tensor(feat_mask).float(),
                            torch.tensor(module_mask).float(),
                        )
    ...
    def train(
        ...
        for i_epoch in range(max_epochs):
            ...
            trainloss = self._train_epoch(
                train_dataloader, pathway_dropout=pathway_dropout, use_c=use_c
            )
            trainloss /= len(train_dataset)
    ...
    def latent_space_names(self, terms=None):
        ...
        if (
            self.model.add_auxiliary_module
...
class LogisticRegression(torch.nn.Module):
    """
    Simple classifier so RNASeq data.

    Adapted from https://towardsdatascience.com/logistic-regression-with-pytorch-3c8bbea594be
    """
    ...
    def fit(self, X_train, y_train, X_test, y_test, criterion, optimizer, epochs=100):
        losses = []
        losses_test = []
        for epoch in range(int(epochs)):
            ...
            with torch.no_grad():
                ...
                predicted_test = outputs_test.round().cpu().detach().numpy()
                ...
                correct += np.sum(
                    torch.squeeze(outputs).round().cpu().detach().numpy()
